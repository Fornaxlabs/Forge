...
def _proj(**over: Any) -> dict[str, Any]:
    p: dict[str, Any] = {
        ...
        "path": "/home/u/myproj",
        "collected_at": NOW,
        "git": {"branch": "main", "last_commit": "feat: add thing",
                "last_commit_ago": "2 hours ago", "uncommitted_files": 0,
                "has_remote": True, "remote": "git@example.invalid:u/myproj.git"},
        ...
        "secrets": {"tool": "gitleaks", "scope": "full history", "findings": 0,
                    ...
                    "pre_push_gate": True, "note": "quick scan note"},
...
def test_renders_all_projects_and_fleet() -> None:
    out = rd.render([_proj(project="alpha", path="/a"),
                     _proj(project="beta", path="/b")], NOW)
...
def test_xss_escaped_in_every_string_field() -> None:
    p = _proj(
        project=XSS, path=XSS,
        git={"branch": XSS, "last_commit": XSS, "last_commit_ago": XSS,
             "remote": XSS, "has_remote": True},
    ...
    p["certificate"] = {
        "level": "L1", "commit": XSS, "governed_runs": 0,
        "generated_at": NOW, "honest_note": XSS,
        "claims": [{"name": XSS, "passed": False, "evidence": XSS,
                    "reproduce": XSS}],
    }
    p["runs"] = [{"run_id": XSS, "task": XSS, "started": XSS,
                  "outcome": XSS, "events": 2, "href": XSS}]
    out = rd.render([p], XSS)
...
def test_xss_escaped_in_connected_github_fields() -> None:
    ...
    p = _proj(github={"connected": True, "open_prs": 1,
                      "pr_titles": [XSS], "latest_ci": XSS, "ci_commit": XSS})
    out = rd.render([p], NOW)
    assert "<script>alert" not in out
    assert "<img src=x" not in out
    assert "&lt;script&gt;alert" in out  # payload present, but escaped
...
def test_xss_never_breaks_out_of_attributes() -> None:
    p = _proj()
    p["runs"] = [{"run_id": "r", "task": "t", "started": "", "outcome": "green",
                  "events": 2, "href": '"><script>alert(3)</script>'}]
    out = rd.render([p], NOW)
...
def test_certificate_levels_render() -> None:
    base = _proj()
    base["certificate"] = {"level": "L3", "claims": [
        {"name": "enforcement-armed", "passed": True, "evidence": "e",
         "reproduce": "r"}], "governed_runs": 2, "commit": "abc123",
        "generated_at": NOW, "honest_note": "note"}
    out = rd.render([base], NOW)
    assert "Forge Verified — Provenanced" in out
    assert "1/1 claims pass" in out

    none = _proj()
    none["certificate"] = {"level": "none", "claims": [], "governed_runs": 0,
                           "commit": "abc", "generated_at": NOW, "honest_note": "n"}
...
def test_runs_absent_vs_empty_vs_linked() -> None:
    ...
    linked = _proj()
    linked["runs"] = [{"run_id": "2026-07-18-084238-refund",
                       "task": "Add refund endpoint", "started": NOW,
                       "outcome": "escalated", "events": 16,
    ...
    unlinked = _proj()
    unlinked["runs"] = [{"run_id": "r1", "task": "t", "started": "",
                         "outcome": "", "events": 1}]
...
def test_gates_meter_counts_and_missing_labels() -> None:
    ...
    two_off = rd.render([_proj(enforcement={"ci_workflow": False,
                                            "plan_mode_first": False})], NOW)
...
def test_guard_tester_js_covers_every_guard_deny_concept() -> None:
    ...
    for token in ("rm", "--force", "--recursive", "git", "push",
                  "--force-with-lease", "chmod", "777", "fork bomb",
                  "--no-preserve-root"):
...
def test_no_external_references() -> None:
    out = rd.render([_proj(project="alpha", path="/a"),
                     _proj(project="beta", path="/b",
                           secrets={"findings": 1, "clean": False})], NOW)
