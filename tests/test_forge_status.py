...
def _fake_gitleaks(bindir: Path, findings: list[dict[str, str]] | str,
...
def test_probe_enforcement_empty_dir(tmp_path: Path) -> None:
    ...
    assert set(enf) == {
        "pre_commit_config", "pre_commit_installed", "pre_push_gate",
        "ci_workflow", "gitleaks_allowlist", "plan_mode_first",
    }
...
def test_gitleaks_scan_timeout(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    bindir = tmp_path / "bin"
    _fake_tool(bindir, "gitleaks", "import time\ntime.sleep(5)\n")
    monkeypatch.setenv("PATH", f"{bindir}:{_SYS_PATH}")
    assert fs._gitleaks_scan(["gitleaks", "git"], str(tmp_path), timeout=0.2) == "timeout"
...
def test_probe_secrets_tool_not_installed(
    ...
    assert fs.probe_secrets(str(tmp_path), history=False) == {
        "tool": "gitleaks", "status": "not installed"}
...
def test_probe_deps_vulns_found(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    bindir = tmp_path / "bin"
    _fake_tool(bindir, "pip-audit",
...
def test_probe_deps_unresolvable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    bindir = tmp_path / "bin"
...
def test_probe_github_not_installed(
    ...
    assert fs.probe_github(str(tmp_path)) == {
        "connected": False, "reason": "gh not installed"}
...
def test_probe_github_gh_error(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    bindir = tmp_path / "bin"
...
def test_main_json_and_text(tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
...
def test_fmt_findings_line() -> None:
    s: dict[str, Any] = {
        ...
        "secrets": {"tool": "gitleaks", "scope": "full history", "findings": 3,
                    "noise_excluded": 0, "clean": False, "allowlist": False,
                    "pre_push_gate": False},
        "enforcement": dict.fromkeys(
            ("pre_commit_config", "pre_commit_installed", "pre_push_gate",
             "ci_workflow", "gitleaks_allowlist", "plan_mode_first"), False),
