...
2. TRACE start (after triage — start requires it):
   `TRACE start --task "$ARGUMENTS" --triage <SMALL|MEDIUM|LARGE> --git-ref "$(git rev-parse --short HEAD)"`
   Then log the triage stage with its reason:
...
7. TRACE end: `TRACE end --outcome <green|escalated|abandoned> --iterations N --tool-calls N`
   (clears active_run.json); one-line summary to forge_memory if a durable
   lesson emerged.
